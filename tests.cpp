...
// This class is used in the hashtable tests below.  To allow
// referring to its hash function via explicit global namespacing (as
// in "::hash(myBadHash)") which some people may do, we must declare
// it *before* the definition of the template.  (This is only
// necessary because of the explicit namespacing; if we say
// "hash(myBadHash)", then late declarations are allowed.  But
// implicit namespacing requires us to have our declarations in a row
// before the template.
class BadHash {
    public:
    BadHash(int x) {
...
/* helper check function that verifies that a vector is ordered by the
   second part of its pair elements */
template <typename T, typename U>
void CHECK_VECTOR_ORDER_BY_SECOND(std::vector<pair<T, U>>);

/* helper check function that verifies that two vectors are
   set-equivalent. An item is in the first vector iff it is in the
   second vector.  */
template <typename T>
...
SUITE(hashTable) {
    ...
    TEST(hashTableCollision) {
        // Unless the table has a hefty, massive starting capacity,
        // one of these passes will trigger a collision.
...
SUITE(keywordSearch) {
    ...
    TEST_KS(Cap10Result2, DataStructuresAndAlgorithmAnalysis, stack,
                ({{140, 24},
                 {142, 17},
                 {141, 15},
                 {146, 10},
